							...
							{   // объект
								...
								{   // обычный объект
									...
									{   // объектов не было
										...
										scen.mTel[0] = new Telo(scen, 0, new String(name, "ASCII"));
									} else
									{
										scen.mTel = Arrays.copyOf(scen.mTel, scen.mTel.length + 1);
										scen.mTel[scen.mTel.length - 1] = new Telo(scen,scen.mTel.length - 1, new String(name, "ASCII"));
									}
									trimeshLenght = l;
									...
									{   // считываем содержимое CHUNK_TRIMESH
										...
										{	// список вершин текущего объекта
											...
											{	// координаты каждой из вершин
												scen.mTel[scen.mTel.length - 1].mp[j] = new Tochka(scen.mTel[scen.mTel.length - 1], j, readFloat(in), readFloat(in), readFloat(in));
											}
										...
										{	// список граней текущего объекта
											...
											{	// список граней
												...
												b = readWord(in);
												c = readWord(in);
												scen.mTel[scen.mTel.length - 1].mg[j] = new Gran(scen.mTel[scen.mTel.length - 1],j, a, b, c);
